...
export default function CustomerLedger() {
  ...
  const fetchLedger = useCallback(async () => {
    setLoading(true);
    try {
      const { data: res } = await ledgerService.getCustomerLedger(customerId, { page, limit });
      if (res.data) {
        setData(res.data);
      }
    } catch (error) {
      showError(error.response?.data?.message || 'Failed to load ledger');
      navigate('/customers');
    } finally {
      setLoading(false);
    }
  ...
  if (!data) {
    return (
      <div className="empty-state">
        ...
        <p>Unable to load the customer ledger.</p>
  ...
  const { customer, summary, ledger, pagination } = data;
    ...
    <div className="ledger-page">
      ...
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h2><i className="fa-solid fa-book"></i> Customer Ledger</h2>
            <p style={{ margin: 0, color: '#888', fontSize: '13px' }}>{customer.name} • {customer.phone || 'No phone'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="ledger-summary" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', marginBottom: '20px' }}>
        <div className="ledger-summary-card" style={{ background: '#f0fdf4', borderRadius: '12px', padding: '16px', border: '1px solid #bbf7d0' }}>
          <div style={{ fontSize: '11px', color: '#166534', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 600 }}>Total Sales</div>
          <div style={{ fontSize: '24px', fontWeight: 700, color: '#16a34a', marginTop: '4px' }}>
            <AnimatedCounter value={summary.totalSales} duration={800} />
          </div>
        </div>
                            ...
                            <table style={{ width: '100%', fontSize: '12px', borderCollapse: 'collapse' }}>
                              <thead>
                                <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                                  ...
                                  <th style={{ padding: '6px 8px', textAlign: 'right', color: '#888' }}>Previous Due</th>
                                  <th style={{ padding: '6px 8px', textAlign: 'right', color: '#888' }}>Remaining Due</th>
